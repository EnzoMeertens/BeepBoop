...
////
	if (NT_SUCCESS(ntStatus))
	{
	    ...
	    m_lRecPos = 0;
        m_lDataLen = 0;
		m_pvTranBuf = (PVOID)ExAllocatePoolWithTag( NonPagedPool, TRAN_BUFFER_SIZE, PALEBLUEAUDIO_POOLTAG );
		if (!m_pvTranBuf)
		{
...
*/
{
  ...
  if (NT_SUCCESS(ntStatus)) {
    stream = new (NonPagedPool, PALEBLUEAUDIO_POOLTAG) CMiniportWaveCyclicStream(OuterUnknown);
    if (stream) {
      stream->AddRef();
  ...
  if (PropertyRequest->Verb & KSPROPERTY_TYPE_BASICSUPPORT) {
    ...
    if (NT_SUCCESS(ntStatus)) {
      if (PropertyRequest->Verb & KSPROPERTY_TYPE_GET) {
        ...
        pComponentId->Product   = PID_PALEBLUEAUDIO;
        pComponentId->Name      = NAME_PALEBLUEAUDIO;
        pComponentId->Component = GUID_NULL; // Not used for extended caps.
        pComponentId->Version   = PALEBLUEAUDIO_VERSION;
        pComponentId->Revision  = PALEBLUEAUDIO_REVISION;

...
#define DBGMESSAGE "[PALEBLUE-Audio] wavestream.cpp: "
...
*/
{
  ...
  //m_pvDmaBuffer = (PVOID) ExAllocatePoolWithTag(NonPagedPool, BufferSize, PALEBLUEAUDIO_POOLTAG);
  //if (!m_pvDmaBuffer) {
  //    ntStatus = STATUS_INSUFFICIENT_RESOURCES;
  //} else {
...

{
 ...
 /////
	if( ByteCount > m_pMiniport->m_ulTranBufSize ) {
		DBGPRINT("Byte count too large for dma buffer.\n");
		return;
	}
	//////
	KeAcquireSpinLock( &m_pMiniport->spin_lock , &m_pMiniport->spin_irql );

...
class CAdapterCommon : public IAdapterCommon, public IAdapterPowerManagement, public CUnknown {
	private:
        ...
        PCPALEBLUEAudioHW           m_pHW;          // VAD HW object
...
--*/
{
    ...
    // Initialize HW.
    // 
    m_pHW = new (NonPagedPool, PALEBLUEAUDIO_POOLTAG)  CPALEBLUEAudioHW;
    if (!m_pHW)
    {
...
--*/
{
    if (m_pHW)
    {
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "Volume %ld\n", Value);
